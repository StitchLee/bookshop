...
<div class=" admin-content">
		...
		<div class="daohang">
			<ul>
				...
				<li><button type="button" class="am-btn am-btn-default am-radius am-btn-xs">商品管理<a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close="">×</a></li>	
				<li><button type="button" class="am-btn am-btn-default am-radius am-btn-xs">商品列表<a href="javascript: void(0)" class="am-close am-close-spin" data-am-modal-close="">×</a></li>
			</ul>	
</div>
  ...
  <ul>
    <li>
      ...
      <!--  <c:forEach items="${booklist}" var="book">
			...
			<option value="<%=path%>/typelist.do?typeId=${book.booktype.booktypeId}">${book.booktype.booktypeName}</option>
    ...
    <form class="am-form am-g">
       ...
       <tr>
            <td colspan="6" align="center" bgcolor="#5BA8DE">共${page.totalRecords}条记录 共${page.totalPages}页 当前第${page.pageNo}页<br>            
                <a href="<%=request.getContextPath()%>/page.do?pageNo=${page.topPageNo}"><input type="button" name="fristPage" value="首页" /></a>
                <c:choose>
                  <c:when test="${page.pageNo!=1}">             
                      <a href="<%=request.getContextPath()%>/page.do?pageNo=${page.previousPageNo }"><input type="button" name="previousPage" value="上一页" /></a>                
                  </c:when>
                  <c:otherwise>   
                ...
                <c:choose>
                  <c:when test="${page.pageNo != page.totalPages}">
                    <a href="<%=request.getContextPath()%>/page.do?pageNo=${page.nextPageNo }"><input type="button" name="nextPage" value="下一页" /></a>
                  </c:when>
                  <c:otherwise>    
                      <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                  </c:otherwise>
                </c:choose>
                <a href="<%=request.getContextPath()%>/page.do?pageNo=${page.bottomPageNo}"><input type="button" name="lastPage" value="尾页" /></a>
            </td>
        </tr>
     </table>
