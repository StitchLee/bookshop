...
<div class="admin-biaogelist">
  ...
  <ul>
    <li>
      <div class="am-btn-group am-btn-group-xs">
        <select data-am-selected="{btnWidth: 90, btnSize: 'sm', btnStyle: 'default'}">
          ...
          <option value="<%=path%>/typelist.do?typeId=1">小说类</option>
          <option value="<%=path%>/typelist.do?typeId=2">科技类</option>
           <option value="<%=path%>/typelist.do?typeId=3">历史类</option> 
          <option value="<%=path%>/typelist.do?typeId=4">儿童文学类</option>  
        </select>
      </div>
    </li>
  </ul>
    ...
    <form method="post"  action="<%=path%>/delete.do">
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
            ...
            <tbody>
              	<c:forEach items="${booklist}" var="book">
					<tr> 
						<td><input type="checkbox" value= "${book.bookId}" name = "bookIds"></td>
						<td>${book.bookId}</td>
						<td>${book.bookName}</td>
         ...
         <input type="submit" class="am-btn am-btn-success am-radius" value="删除">
       </form>   
       ...
       <tr>
            <td colspan="6" align="center" bgcolor="#5BA8DE">共${page.totalRecords}条记录 共${page.totalPages}页 当前第${page.pageNo}页<br>            
                <a href="<%=request.getContextPath()%>/deletelist.do?pageNo=${page.topPageNo}"><input type="button" name="fristPage" value="首页" /></a>
                <c:choose>
                  <c:when test="${page.pageNo!=1}">             
                      <a href="<%=request.getContextPath()%>/deletelist.do?pageNo=${page.previousPageNo }"><input type="button" name="previousPage" value="上一页" /></a>                
                  </c:when>
                  <c:otherwise>   
                ...
                <c:choose>
                  <c:when test="${page.pageNo != page.totalPages}">
                    <a href="<%=request.getContextPath()%>/deletelist.do?pageNo=${page.nextPageNo }"><input type="button" name="nextPage" value="下一页" /></a>
                  </c:when>
                  <c:otherwise>    
                      <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                  </c:otherwise>
                </c:choose>
                <a href="<%=request.getContextPath()%>/deletelist.do?pageNo=${page.bottomPageNo}"><input type="button" name="lastPage" value="尾页" /></a>
            </td>
        </tr>
     </table>
