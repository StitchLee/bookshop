    ...
    <div id="templatemo_content">
        ...
        <div id="templatemo_content_left">
         ...
         <div  style="margin-life: 100px;">
        	<select data-am-selected="{btnWidth: 90, btnSize: 'sm', btnStyle: 'default'}" onchange="window.location=this.value;">
          		...
          		<option value="<%=path%>/booktypepage.do?typeid=1&name=${name}&id=${newbook.bookId}">小说类</option>
          		<option value="<%=path%>/booktypepage.do?typeid=2&name=${name}&id=${newbook.bookId}">科技类</option> 
          		<option value="<%=path%>/booktypepage.do?typeid=3&name=${name}&id=${newbook.bookId}">历史类</option> 
          		<option value="<%=path%>/booktypepage.do?typeid=4&name=${name}&id=${newbook.bookId}">儿童文学类</option> 
        	</select>
      	</div>
        <div id="templatemo_content_right">
        	<c:forEach var="book" items="${booklist}" varStatus="status">
        		<c:if test="${status.index%2==0}">
                    ...
                    <div class="buy_now_button"><a href="<%=path%>/bookcar.do?bookid=${book.bookId}&name=${name}">Buy Now</a></div>
       ...
       <tr>
            <td colspan="6" align="center" bgcolor="#5BA8DE">共${page.totalRecords}条记录 共${page.totalPages}页 当前第${page.pageNo}页<br>            
                <a href="<%=request.getContextPath()%>/booktypepagetwo.do?pageNo=${page.topPageNo}&typeid=${typeid}&name=${name}"><input type="button" name="fristPage" value="首页" /></a>
                <c:choose>
                  <c:when test="${page.pageNo!=1}">             
                      <a href="<%=request.getContextPath()%>/booktypepagetwo.do?pageNo=${page.previousPageNo }&typeid=${typeid}&name=${name}"><input type="button" name="previousPage" value="上一页" /></a>                
                  </c:when>
                  <c:otherwise>   
                ...
                <c:choose>
                  <c:when test="${page.pageNo != page.totalPages}">
                    <a href="<%=request.getContextPath()%>/booktypepagetwo.do?pageNo=${page.nextPageNo }&typeid=${typeid}&name=${name}"><input type="button" name="nextPage" value="下一页" /></a>
                  </c:when>
                  <c:otherwise>    
                      <input type="button" disabled="disabled" name="nextPage" value="下一页" />
                  </c:otherwise>
                </c:choose>
                <a href="<%=request.getContextPath()%>/booktypepagetwo.do?pageNo=${page.bottomPageNo}&typeid=${typeid}&name=${name}"><input type="button" name="lastPage" value="尾页" /></a>
            </td>
        </tr>
     </table>
